  ...
  <div class="container-fluid d-flex justify-content-center" id="main">
    <div class="w-50">
      <div class="input-group mb-3 mt-2">
        <input type="emai" class="form-control" v-model="user" id="newMail" aria-label="newMail" aria-describedby="basic-0">
    ...
    <div class="mb-5 mt-3">
      <template v-if="newTotal!=0">
        ...
        <div class="form-floating mb-3">
          <input type="text" id="Reservation averageDays" :value="totalDays/days.length + ' giorni'" class="form-control" aria-label="Reservation averageDays" aria-describedby="basic-4" readonly>
          <label for="Reservation averageDays"> Media durata noleggio</label>
        </div>
      </template>
      <template v-else>
        Il cliente non esiste o non ha effettuato prenotazioni
      </template>
    </div>
    </div>
  </div>
